...
def get_dau_ky(db: Session = Depends(get_db)):
    ...
    cong_no_khach = db.execute("SELECT ma_kh, so_du FROM cong_no_khach_hang").fetchall()
    cong_no_ncc = db.execute("SELECT ma_ncc, so_du FROM cong_no_ncc").fetchall()

    return {
        "ton_kho": [{"ma_kho": x.ma_kho, "ma_sp": x.ma_sp, "so_luong": float(x.so_luong)} for x in ton_kho],
        "quy_nhan_vien": [{"ma_nv": x.ma_nv, "so_du": float(x.so_du)} for x in quy_nv],
        "quy_cong_ty": {
            "tien_mat": float(quy_ct.tien_mat) if quy_ct else 0,
            "tien_ngan_hang": float(quy_ct.tien_ngan_hang) if quy_ct else 0
        },
        "cong_no_khach": [{"ma_kh": x[0], "so_du": float(x[1])} for x in cong_no_khach],
        "cong_no_ncc": [{"ma_ncc": x[0], "so_du": float(x[1])} for x in cong_no_ncc]
    }
